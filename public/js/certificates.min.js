const getBasePath = () => {
    ...
    return e.match(/\/pages\/[^/]+\/[^/]*$/) ? '../../' : e.includes('/pages/') ? '../' : '';
  },
  escapeHtml = (e) => {
...
class CertificateModal {
  constructor() {
    ...
    const e = 'undefined' != typeof process && 'test' === process.env.NODE_ENV;
    this.container
      ? (e || console.log('🔍 Modal container found:', this.container), this.init())
      : e || console.error('Certificate modal container not found!');
  }
  init() {
  ...
  generateCertificateGrid() {
    ...
    return this.certificates
      .map(
        (t) =>
          `\n      <div class="cert-card" data-id="${t.id}" onclick="window.certificateModal.openCertificate(${t.id - 1})">\n        <div class="cert-card-image">\n          <img src="${e}${escapeHtml(t.thumb)}" alt="${escapeHtml(t.title)}" loading="lazy" decoding="async">\n          <div class="cert-card-overlay">\n            <i class="material-icons">visibility</i>\n            <span>View Certificate</span>\n          </div>\n        </div>\n        <div class="cert-card-content">\n          <h4>${escapeHtml(t.title)}</h4>\n          <span class="cert-badge">${escapeHtml(t.category)}</span>\n          ${t.linkedinUrl ? '<i class="fab fa-linkedin cert-linkedin-icon" title="Available on LinkedIn"></i>' : ''}\n        </div>\n      </div>\n    `
      )
      .join('');
  }
  attachEventListeners() {
    document.querySelectorAll('.modal-trigger').forEach((e) => {
      '#modal1' === e.getAttribute('href') &&
        (e.onclick = (e) => {
          (e.preventDefault(), this.open());
        });
    });
    ((this.container.querySelector('.cert-close').onclick = () => this.close()),
      (this.container.onclick = (e) => {
        e.target === this.container && this.close();
      }),
      ...
      document.addEventListener('keydown', (e) => {
        ...
        const t = 'none' !== document.getElementById('certViewer').style.display;
        ('Escape' === e.key && this.close(),
          t && 'ArrowLeft' === e.key && this.navigate(-1),
          t && 'ArrowRight' === e.key && this.navigate(1),
          t && 'Backspace' === e.key && (e.preventDefault(), this.showGrid()));
      }));
  }
  open() {
  ...
  close() {
    (this.container.classList.remove('active'), (document.body.style.overflow = ''));
  }
  showGrid() {
  ...
  navigate(e) {
    ((this.currentIndex = (this.currentIndex + e + certificates.length) % certificates.length),
      this.updateViewer());
  }
  updateViewer() {
    const e = certificates[this.currentIndex],
      t = getBasePath(),
      i = document.getElementById('certImage');
    ((i.src = t + e.image),
      (i.alt = e.title),
      (document.getElementById('certTitle').textContent = e.title),
      (document.getElementById('certCategory').textContent = e.category),
      (document.getElementById('currentCert').textContent = this.currentIndex + 1),
      (document.getElementById('viewFullBtn').href = t + e.image));
    const a = document.getElementById('linkedinBtn');
    e.linkedinUrl
      ? ((a.href = e.linkedinUrl), (a.style.display = 'inline-flex'))
...
if ('undefined' != typeof window) {
  ...
  const e = 'undefined' != typeof process && 'test' === process.env.NODE_ENV;
  (e || console.log('🔍 certificates.js loaded'),
    e ||
